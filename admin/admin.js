...
(function () {
  ...
  function renderEditor() {
    ...
    loadBody(slug).then(function (body) {
      ...
      editorBox.appendChild(h('div', { class: 'slugline' }, 'Адреса:', h('code', null, '#/' + slug),
  ...
  function helpContent() {
    return h('div', { class: 'panel help-body' }, h('div', { class: 'panel-b' },
      h('h2', { class: 'section-title' }, 'Як користуватися'),
      h('h3', null, 'Щоденна робота'),
      h('ol', { class: 'steps' },
        h('li', null, 'Відкрийте вкладку «Сторінки та меню», виберіть сторінку й змініть текст.'),
        h('li', null, 'Щоб додати документ чи фото, натисніть «Файл» або «Зображення» над полем тексту й завантажте файл.'),
        h('li', null, 'Натисніть жовту кнопку «Опублікувати зміни». Сайт оновиться приблизно за 1–2 хвилини.')),
      h('h3', null, 'Меню'),
      h('p', null, 'Кнопки ↑ ↓ змінюють порядок, → робить пункт підпунктом попереднього, ← виносить його на рівень вище. Нові пункти додаються після вибраного.'),
      h('h3', null, 'Як отримати ключ доступу (токен)'),
      tokenSteps(),
      h('h3', null, 'Безпека'),
      h('p', null, 'Токен дає право змінювати сайт — не передавайте його іншим. Якщо токен потрапив не до тих рук, видаліть його на GitHub (Settings → Developer settings → Fine-grained tokens). Кожен адміністратор має свій обліковий запис GitHub, доданий до репозиторію (Settings → Collaborators).')));
  }
  function tokenSteps() {
